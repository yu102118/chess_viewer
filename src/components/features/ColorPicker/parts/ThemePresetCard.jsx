...
const ThemePresetCard = memo(function ThemePresetCard({
  ...
  return <button onClick={() => onClick(themeKey, theme)} onMouseEnter={() => onHover(theme)} onMouseLeave={onLeave} className={`
        group relative px-3 py-4 rounded-xl text-xs text-gray-300 
        transition-all duration-300 overflow-hidden
        ${isActive ? 'bg-blue-600/30 border-2 border-blue-500 scale-105' : 'bg-gray-900/80 border-2 border-gray-700 hover:border-blue-500/50'}
        ${isHovered ? 'scale-105' : ''}
      `}>
        ...
        {isActive && <div className="absolute top-1 right-1 w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center">
  ...
  return prevProps.isActive === nextProps.isActive && prevProps.isHovered === nextProps.isHovered && prevProps.themeKey === nextProps.themeKey;
});
