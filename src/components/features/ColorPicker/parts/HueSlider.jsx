  ...
  return <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full border-2 border-gray-700 shadow-lg transition-all duration-200 relative" style={{
          background: getCurrentColor(currentHue),
          transform: isDragging ? 'scale(1.15)' : 'scale(1)',
          boxShadow: isDragging ? `0 0 20px ${getCurrentColor(currentHue)}80` : '0 2px 8px rgba(0,0,0,0.3)'
        }}>
              {isDragging && <div className="absolute inset-0 rounded-full animate-ping opacity-50" style={{
            background: getCurrentColor(currentHue)
          }} />}
            </div>
        ...
        <div className="relative">
          <div className="relative h-14 rounded-xl overflow-hidden border-2 border-gray-700 shadow-inner bg-gray-900">
            <input type="range" min="0" max="360" step="1" value={currentHue} onChange={onChange} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onTouchStart={handleMouseDown} onTouchEnd={handleMouseUp} className="hue-slider w-full h-full appearance-none cursor-pointer bg-transparent relative z-10" />
          </div>

          <div className="flex justify-between mt-2.5 px-1">
            {degreeMarkers.map(degree => <div key={degree} className="flex flex-col items-center">
                <div className={`h-2 rounded-full transition-all ${Math.abs(currentHue - degree) < 10 ? 'w-0.5 bg-blue-500' : 'w-px bg-gray-600'}`} />
                <span className={`text-[10px] font-mono mt-1 transition-colors ${Math.abs(currentHue - degree) < 10 ? 'text-blue-400 font-bold' : 'text-gray-500'}`}>
                  {degree}°
                </span>
              </div>)}
          </div>
