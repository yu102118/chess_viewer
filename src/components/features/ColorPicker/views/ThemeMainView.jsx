...
const ThemePresetButton = memo(({ themeKey, theme, isActive, onClick }) => {
  ...
  return (
    <button
      ...
      className={`group relative p-2 rounded-xl transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 hover:scale-[1.02] overflow-hidden ${
        isActive
...
const CustomThemeCard = memo(({ isActive, onClick }) => {
  return (
    ...
    >
      <div className="relative">
        <div
          className="flex w-full h-12 rounded-lg overflow-hidden shadow-sm bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500"
          aria-hidden="true"
        />
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm rounded-lg flex items-center justify-center translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-out">
