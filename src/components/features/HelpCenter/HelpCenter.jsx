...
import { X, BookOpen, HelpCircle, Info, Shield, FileText, Settings as SettingsIcon, Search } from 'lucide-react';
const SECTIONS = {
  FEATURES: 'features',
  ABOUT: 'about',
  SUPPORT: 'support',
...
const HelpCenterDrawer = memo(function HelpCenterDrawer({
  ...
  const renderContent = () => {
    if (searchQuery.trim() && filteredContent) {
      return <div className="space-y-4">
          ...
          {filteredContent.length === 0 ? <div className="text-center py-12">
            ...
            </div> : filteredContent.map(item => <div key={`${item.sectionId}-${item.title}`} className="bg-surface rounded-lg p-5 border border-border">
    ...
    return <div className="space-y-6">
        ...
        <div className="space-y-4">
          {content.sections.map(section => <div key={section.title} className="bg-surface rounded-lg p-5 border border-border hover:border-accent/50 transition-colors">
  ...
  return <>
      {}
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-300" onClick={onClose} aria-hidden="true" />

      {}
      <div className="fixed inset-y-0 right-0 z-50 w-full md:w-[50%] lg:w-[55%] xl:w-[45%] min-w-[320px] max-w-[900px] bg-bg border-l border-border shadow-2xl flex flex-col animate-slide-in-right">
        {}
        <div className="px-6 py-4 border-b border-border bg-surface">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface-hover transition-colors" aria-label="Close help center">
          ...
          <div className="relative">
            ...
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search help articles..." className="w-full pl-10 pr-4 py-2.5 bg-bg border border-border rounded-lg text-sm text-text-primary placeholder-text-secondary focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent transition-all" />
          </div>
        </div>
        ...
        <div className="border-b border-border bg-surface-elevated overflow-x-auto">
          <div className="flex gap-1 px-4 min-w-max">
            ...
            return <button key={tab.id} onClick={() => {
              ...
              setSearchQuery('');
            }} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium transition-all border-b-2 ${isActive ? 'text-accent border-accent' : 'text-text-secondary border-transparent hover:text-text-primary hover:bg-surface-hover'}`} aria-label={tab.label} title={tab.label}>
