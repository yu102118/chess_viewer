              ...
              >
                {intervalOptions.map((option) => (
                  <button
                    ...
                    className={`w-full px-4 py-2 text-sm text-left transition-colors ${interval === option.value ? 'bg-blue-600 text-white font-semibold' : 'text-gray-300 hover:bg-gray-700'}`}
                    role="menuitem"
                  >
                    {option.label}
                  </button>
          ...
          <div className="flex gap-2" role="tablist">
            {validFens.map((fenVal, idx) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all ${idx === currentIndex ? 'bg-blue-500 w-8' : 'bg-gray-600 hover:bg-gray-500 w-2.5'}`}
