      ...
      <div className="relative">
        <button
          ...
          className={`
            ...
            ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-border hover:bg-surface-hover'}
          `}
        >
          <span
        ...
        {isOpen && (
          <>
            ...
            <ul
              ...
              role="listbox"
              aria-labelledby={label ? `${selectId}-label` : undefined}
              className="absolute top-full left-0 right-0 mt-1 bg-surface-elevated/95 backdrop-blur-lg border border-border rounded-xl shadow-lg z-30 max-h-60 overflow-y-auto animate-scale-in"
            >
              {options.map((option) => (
                <li
                  key={option.value}
                  ...
                  className={`
                    ...
                    ${option.value === value ? 'bg-accent text-bg' : 'text-text-secondary hover:bg-surface-hover hover:text-text-primary'}
                  `}
                >
