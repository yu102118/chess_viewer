...
import { parseFEN, drawCoordinates, getCoordinateParams, rafThrottle, logger } from '@/utils';
...
const ChessBoard = React.forwardRef(function ChessBoard(props, ref) {
  ...
  const drawBoard = useCallback(() => {
    ...
    if (showCoords) {
      ...
      ctx.lineWidth = 1;
      ctx.strokeRect(borderSize - 0.5, borderSize - 0.5, boardSize + 1, boardSize + 1);
    }
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        const bounds = getSquareBounds(displayRow, displayCol, borderSize, squareSize);
    ...
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 8; col++) {
        ...
        if (img && img.complete && img.naturalWidth > 0) {
          ...
          const displayCol = flipped ? 7 - col : col;
          const bounds = getSquareBounds(displayRow, displayCol, borderSize, squareSize);
          const pieceSize = Math.round(Math.min(bounds.width, bounds.height) * 1.0);
  ...
  return <div className="relative inline-block w-full max-w-full" role="img" aria-label={getBoardDescription()}>
      <canvas ref={canvasRef} className="transition-all duration-300 w-full h-auto" style={{
      display: 'block',
      imageRendering: '-webkit-optimize-contrast',
      background: 'transparent'
    }} aria-hidden="true" />
      {isLoading && <div className="absolute inset-0 flex flex-col items-center justify-center bg-surface/90 backdrop-blur-sm rounded-lg" role="status" aria-live="polite">
      ...
      {error && <div className="absolute inset-0 flex items-center justify-center bg-error/10 backdrop-blur-sm rounded-lg border-2 border-error/50" role="alert" aria-live="assertive">
